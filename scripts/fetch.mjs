...
async function discoverSeason() {
  ...
  const wc = (data.Results || []).find(
    (m) => (m.CompetitionName?.[0]?.Description || '').startsWith('FIFA World Cup'),
  );
...
async function main() {
  ...
  const results = compute(draw, data.Results, nowIso);
  console.log(
  ...
  let prevStable = null;
  try { prevStable = stable(JSON.parse(await readFile(OUT_PATH, 'utf8'))); } catch {}
